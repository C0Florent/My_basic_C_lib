...
typedef struct conversion_details {
    ...
    char len_modifier[2];
    int width;
    bool dash_flag;   /* These four variables   */
    bool zero_flag;   /* are either 0 or 1, just */
    bool space_flag;  /* indicating if each flag */
    bool plus_flag;   /* is present or not       */
} conv_det_t;


typedef struct put_conv_ptr {
    int (*fptr)(conv_det_t *details, va_list *arguments, int *count);
    char conversion;
} put_conv_ptr_t;
...
int my_put_conv_b(conv_det_t *details, va_list *arguments, int *count);
int my_put_conv_o(conv_det_t *details, va_list *arguments, int *count);
...
int my_put_conv_g(conv_det_t *details, va_list *arguments, int *count);
int my_put_conv_a(conv_det_t *details, va_list *arguments, int *count);
...
int my_put_conv_n(conv_det_t *details, va_list *arguments, int *count);
int my_put_conv_percent(conv_det_t *details, va_list *arguments, int *count);
